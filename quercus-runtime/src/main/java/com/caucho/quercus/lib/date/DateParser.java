...
private void parseMonthDate(int value1)
  {
    ...
    if (token == INT) {
      ...
      if (token == INT) {
    	  int _save_index = _index;
    	  int _save_value = _value;
    	  token = nextToken();
    	  if (token == ':') { 
    		  // Is a time
    		  token = nextToken(); // minutes
    		  token = nextToken(); // :
    		  token = nextToken(); // seconds
    		  token = nextToken(); 
    		  if (token == INT) {
    			  
    		  } else if (token == '+') { // Time zone
    			  token = nextToken(); // Time difference
    			  token = nextToken(); // Year!!!
    			  
    		  }
    		  _index = _save_index_before;
    	  } else {
    		  _value = _save_value;
    		  _index = _save_index; 
    	  }
